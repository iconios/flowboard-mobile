...
class SecureCookieManager {
  static async setCookie(
    name: string,
    value: string,
    options: { expires?: string; secure: boolean },
  ): Promise<boolean> {
    try {
      const cookieValue = {
        value,
  ...
  static async getCookie(name: string): Promise<UserDataType | null> {
    try {
  ...
  static async deleteCookie(name: string): Promise<boolean | null> {
    try {
      // 1. Get and validate the cookie name received
      if (!name) return null;
